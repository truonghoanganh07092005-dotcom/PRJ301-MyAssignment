...
public abstract class BaseRequiredAuthorizationController
    ...
    private boolean isAuthorized(HttpServletRequest req, User user) {
        ...
        String url = req.getServletPath(); // ví dụ: /request/approve
        for (Role r : user.getRoles()) {
            for (Feature f : r.getFeatures()) {
                if (f.getUrl() == null) continue;
                String fu = f.getUrl().trim();
                // ✅ khớp chính xác hoặc tiền tố
                if (url.equals(fu) || url.startsWith(fu)) return true;
            }
        }
        return false;
    }
