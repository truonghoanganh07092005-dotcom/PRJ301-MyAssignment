...
public class CreateController extends BaseRequiredAuthorizationController {
    ...
    protected void processPost(HttpServletRequest req, HttpServletResponse resp, User user)
        ...
        try { from = LocalDate.parse(fromS); }
        catch (Exception e) { errors.add("Ngày bắt đầu không hợp lệ."); }

        try { to   = LocalDate.parse(toS); }
        catch (Exception e) { errors.add("Ngày kết thúc không hợp lệ."); }

        LocalDate today = LocalDate.now();

        if (from != null && from.isBefore(today))
            errors.add("Từ ngày phải ≥ hôm nay (" + today + ").");
        ...
        creator.setId(user.getId()); // DAO sẽ mapping uid->eid nếu bạn giữ kiểu cũ, hoặc đổi thành getEmployee().getId() nếu đã chuẩn hoá
        ...
        // Nếu không có quyền /request/my thì quay về /home
        ...
        String next = canViewMy ? (ctx + "/request/my?created=" + rid) : (ctx + "/home");
